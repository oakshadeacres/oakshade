...
- **Breeds tab**: card list (first-photo thumbnail, name, specialty star, variety/image counts) with Edit and Delete buttons per card, plus an add-breed row (name input + button; Enter submits). Creating posts the name (server slugifies the id, assigns `order` = max+1; duplicate name → error flash) and opens the new breed's editor directly. Deleting asks for confirmation (notes that photo files stay on disk), then removes the breed file and refreshes the list. Breed editor: basics (name, specialty, order, description), trait rows, varieties & pricing rows (renaming a variety rewrites matching `images[].variety` tags), availability board — three columns (Available/Waitlist/Unavailable) where varieties are dragged between buckets by a handle, click-to-rename, ×-remove, +-add — and the gallery image manager.
